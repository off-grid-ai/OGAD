...
<body>
  ...
  <script>
    ...
    function directState(state,message=''){speaking=state==='loading'||state==='playing';read.textContent=speaking?'STOP':'READ ALOUD';speechStatus.textContent=message||(state==='loading'?'GENERATING AUDIO':state==='playing'?'PLAYING':'')}
    function releaseBrowserAudio(){if(browserAudio){browserAudio.pause();browserAudio=null}if(browserAudioUrl){URL.revokeObjectURL(browserAudioUrl);browserAudioUrl=''}}
    function stopSpeech(){if(gatewayBase){releaseBrowserAudio();directState('idle')}else parent.postMessage({__ogComicTts:'stop'},'*')}
    function show(index){if(!pages.length){previous.disabled=true;next.disabled=true;return}current=Math.max(0,Math.min(index,pages.length-1));pages.forEach((page,i)=>page.classList.toggle('active',i===current));thumbs.forEach((thumb,i)=>{if(i===current)thumb.setAttribute('aria-current','page');else thumb.removeAttribute('aria-current')});label.textContent=(current+1)+' / '+pages.length;previous.disabled=current===0;next.disabled=current===pages.length-1;thumbs[current]?.scrollIntoView({block:'nearest'});requestAnimationFrame(()=>fitCopy(pages[current]));if(speaking)stopSpeech()}
    ...
    async function connectGateway(){for(let port=7878;port<=7882;port+=1){try{const origin='http://127.0.0.1:'+port;const healthResponse=await fetch(origin+'/health');if(!healthResponse.ok)continue;const health=await healthResponse.json();if(health.openai_compatible!==true)continue;gatewayBase=origin+'/v1';const [voiceResponse,modelResponse]=await Promise.all([fetch(gatewayBase+'/audio/voices'),fetch(gatewayBase+'/models?output_modalities=speech')]);if(!voiceResponse.ok)throw new Error('voices unavailable');const voiceData=await voiceResponse.json();const modelData=modelResponse.ok?await modelResponse.json():{data:[]};voices=(Array.isArray(voiceData.voices)?voiceData.voices:[]).map((id)=>({id,label:id,language:'und',languageLabel:'Automatic'}));language.innerHTML='<option value="und">Automatic</option>';language.disabled=true;model.innerHTML='';const speechModels=(Array.isArray(modelData.data)?modelData.data:[]).filter((item)=>item.kind==='speech');(speechModels.length?speechModels:[{id:'local',name:'Off Grid AI'}]).forEach((item)=>{const option=document.createElement('option');option.value=item.id;option.textContent=item.name||item.id;model.append(option)});model.disabled=true;fillVoices();speechStatus.textContent='';return}catch{}}language.innerHTML='<option>OFFLINE</option>';model.innerHTML='<option>OFFLINE</option>';voice.innerHTML='<option>OFFLINE</option>';speechStatus.textContent='OPEN OFF GRID AI DESKTOP'}
    async function speakCurrent(){const text=pages[current]?.querySelector('.story p')?.textContent?.trim()||pages[current]?.querySelector('.notes p')?.textContent?.trim();if(!text)return;if(!gatewayBase){parent.postMessage({__ogComicTts:'speak',text,voice:voice.value},'*');return}releaseBrowserAudio();directState('loading');try{const response=await fetch(gatewayBase+'/audio/speech',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({input:text,voice:voice.value,model:model.value})});if(!response.ok)throw new Error('speech failed');browserAudioUrl=URL.createObjectURL(await response.blob());const audio=new Audio(browserAudioUrl);browserAudio=audio;audio.onended=()=>{releaseBrowserAudio();directState('idle');if(current<pages.length-1){show(current+1);requestAnimationFrame(speakCurrent)}};audio.onerror=()=>{releaseBrowserAudio();directState('error','SPEECH FAILED')};directState('playing');await audio.play()}catch{releaseBrowserAudio();directState('error','SPEECH FAILED')}}
    language.addEventListener('change',fillVoices);model.addEventListener('change',()=>{if(!gatewayBase)parent.postMessage({__ogComicTts:'model',model:model.value},'*')});read.addEventListener('click',()=>{if(speaking){stopSpeech();return}void speakCurrent()});
    ...
    addEventListener('resize',()=>requestAnimationFrame(()=>fitCopy(pages[current])));if(window===parent){void connectGateway()}else{parent.postMessage({__ogComicTts:'catalog'},'*')}
  </script>
</body>
</html>`
}

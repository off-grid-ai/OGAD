...
describe('makeVisionRailExecutor', () => {
  it('runs the task with the goal and returns the action id as the effect', async () => {
    ...
    expect(host.runTask).toHaveBeenCalledWith('share the deck', 'act_vis', 'act_vis', undefined, undefined, undefined)
  ...
  it('falls back to the action intent when no explicit goal is given', async () => {
    ...
    expect(host.runTask).toHaveBeenCalledWith('share the deck over WhatsApp', 'act_vis', 'act_vis', undefined, undefined, undefined)
  })
  ...
  it('keeps the originating Chat as the task journey', async () => {
    ...
    expect(host.runTask).toHaveBeenCalledWith('share the deck', 'act_vis', 'chat-42', undefined, undefined, undefined)
  })

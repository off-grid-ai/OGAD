 ...
 * switch, pause, step budget) plus the user's supervision IS the reliability;
...
export interface VisionRailHost {
  runTask(goal: string, taskId: string): Promise<VisionTaskResult>
}
...
export function makeVisionRailExecutor(
  ...
  return async (action) => {
    ...
    const result = await host.runTask(goal, action.id)
    if (!result.ok) {

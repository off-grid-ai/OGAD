...
export const NATIVE_TOOL_SPECS: NativeToolSpec[] = [
  ...
  },
  {
    name: 'open_url',
    description:
      "Open a URL, link, or app scheme in the user's OWN browser or app (Chrome/Safari, a mailto: draft, whatsapp://send). Use it whenever the desired end-state is the user simply landing on or watching a page in their own browser, logged in - 'open', 'go to', 'play', or 'watch' X. For a site search like 'play Family Guy on YouTube', pass the results URL, e.g. https://www.youtube.com/results?search_query=Family+Guy, so they land ready to play. It opens the page - it does not click or type for the user. When a task must instead be DONE on the page (log in, fill a form, check in, place an order, download), use web_task.",
    parameters: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'web_task',
    description:
      'Have the assistant OPERATE a website for the user in a controlled, watched in-app browser pane - the desired end-state is a task DONE on the page: logging in, filling a form, checking in for a flight, placing an order, downloading a file, or extracting details. Use it only when the agent must click and type on the user\'s behalf, NOT when the user just wants to open or watch something in their own browser (that is open_url). Describe the whole goal in one call; the assistant drives the page step by step and hands control back to the user for any sign-in, one-time code, or payment.',
    parameters: {
      type: 'object',
      properties: {
        goal: {
...
export function systemHintForPlatform(platform: NodeJS.Platform): string {
  if (platform === 'darwin') {
    return "You can act on the user's Mac: manage calendar events (calendar_create_event, calendar_list_events) and reminders (reminders_create, reminders_list), look up people (contacts_search), and send an iMessage (messages_send) or email (mail_send). Resolve a name to a handle with contacts_search before sending. When the user just wants to land on or watch a page in their OWN browser - open, go to, play, or watch X - use open_url (for a site search like 'play X on YouTube', pass the results URL, e.g. https://www.youtube.com/results?search_query=X); it opens the page and does not click or type. When a task must be DONE on a site instead - log in, fill a form, check in, order, download, extract - use web_task, which operates a controlled watched in-app browser; describe the whole goal in one call and it hands back for any sign-in or payment. For a task that needs to control a desktop app directly (no web version), use computer_task - the user watches and can take over. Prefer the direct tools when they fit. Use ISO 8601 for all times. Anything that creates, sends, or runs a task needs the user's approval; tell them it is pending until they approve."
  }
  if (platform === 'win32') {
    return "You can act on the user's PC through Outlook: create calendar events (calendar_create_event) and tasks (reminders_create), and send an email (mail_send). When the user just wants to land on or watch a page in their OWN browser - open, go to, play, or watch X - use open_url (for a site search like 'play X on YouTube', pass the results URL, e.g. https://www.youtube.com/results?search_query=X); it opens the page and does not click or type. When a task must be DONE on a site instead - log in, fill a form, check in, order, download, extract - use web_task, which operates a controlled watched in-app browser; describe the whole goal in one call and it hands back for any sign-in or payment. For a task that needs to control a desktop app directly, use computer_task - the user watches and can take over. Prefer the direct tools when they fit. Use ISO 8601 for all times. There is no message or contact lookup tool on Windows. Anything that creates, sends, or runs a task needs the user's approval; tell them it is pending until they approve."
  }
  return ''
}

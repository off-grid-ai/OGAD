...
export function buildPlannerPrompt(
  ...
  return [
    ...
    '',
    'Rules:',
    '- Route a web request by its desired END-STATE:',
    '   - To OPEN or GO TO a site in the user\'s own browser, use open_url with the URL.',
    "   - To PLAY or WATCH X on a site (e.g. 'play Family Guy on YouTube'), use TWO steps: (1) open_url with the site's search-results URL, e.g. https://www.youtube.com/results?search_query=Family+Guy, so the user's own browser lands on the results, then (2) computer_task with goal 'click the first Family Guy video to play it' so the assistant clicks the result in that same browser.",
    '   - To DO a task ON a page (log in, fill a form, check in, place an order, download, extract), use web_task, which operates a controlled in-app browser.',

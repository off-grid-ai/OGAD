...
# Keep Whisper on the same pinned source as macOS. Pin the FFmpeg release asset by
# tag and digest so a later upstream autobuild cannot silently change an installer.
ROOT="${OFFGRID_BUILD_ROOT:-$(cd "$(dirname "$0")/.." && pwd)}"
WHISPER_REF=v1.7.4
WHISPER_COMMIT=8a9ad7844d6e2a10cddf4b92de4089d7ac2b14a9
...
cmake -S "$WORK/whisper" -B "$WORK/whisper-build" \
...
cmake --build "$WORK/whisper-build" --config Release \
  --parallel "${OFFGRID_BUILD_JOBS:-4}" --target whisper-cli
WHISPER_BIN="$(find "$WORK/whisper-build" -name whisper-cli -type f -perm -111 -print -quit)"
test -n "$WHISPER_BIN"
...
mkdir -p "$BIN/whisper" "$BIN/licenses"
install -m 755 "$WHISPER_BIN" "$BIN/whisper/whisper-cli"
...
for executable in "$BIN/whisper/whisper-cli" "$BIN/ffmpeg"; do
...
echo '[build-voice-linux] staged Whisper and FFmpeg'

...
- **Schedule: a 12-working-day plan**, starting Wednesday, August 13, 2026; first release about day 4. R1 and R2 are cross-platform (macOS + Windows) from day 1. R3 and R4 land macOS-first, with Windows parity for the two heavy native rails - routines (UI Automation) and vision (SendInput) - as a short fast-follow after day 12. The released core stays cross-platform; only the risky, watched rails trail on the second OS.
- **The alternative:** locking Windows step-for-step on every rail inside the line pushes it to about 15 working days. We take the 12-day plan and treat Windows heavy-rail parity as the immediate fast-follow.
...
## Timeline at a glance (12 working days)
...
| R3. Routines | 8 - 10 | Record-by-showing + faithful replay, per-step verified. | macOS-first (AX + CGEvent); Windows UIA fast-follow | Record a routine by showing it once; it replays with per-step verify; a slot resolves from memory. |
| R4. Hard cases | 11 - 12 | Browser rail (shared) + vision rail + safety pass. | macOS + shared browser on both; Windows vision-input fast-follow | A web check-in with takeover; the WhatsApp file-share supervised; drift recovery via vision; safety checklist. |
| Windows heavy-rail parity | fast-follow | UI Automation routines + SendInput vision input | Windows | Routines record/replay and vision-driven steps reach parity on Windows. |
...
- **Windows build + sign + engine setup** (electron-builder target, code-signing, the `llama-server` Windows build), folded in here as the one-time cost, before any Windows rail work depends on it.
...
The magic, and the safest thing to add: memory + LLM + read-only context, no risky automation. It makes the released tool proactive, and it is cross-platform for free (no native rail change).
...
## R3 - routines: demonstration recorder + faithful replay (Days 8 - 10)

Record-by-showing and reliable replay on well-behaved apps. macOS-first; the Windows UI Automation adapter is the fast-follow.
...
**Checkpoint (day 10):** on macOS, record a small routine on a native app by showing it once; it replays faithfully with per-step verification, and a marked slot resolves from memory at run time. The Windows UIA + SendInput adapter follows in the fast-follow.

## R4 - hard cases: browser rail + vision rail (Days 11 - 12)
...
**Checkpoint (day 12):** on macOS, a reasoned web check-in runs in-pane with a takeover at login; the WhatsApp file-share task runs supervised end to end; a demonstrated routine recovers from a drifted step via vision; the safety checklist passes. The browser rail already works on Windows; the Windows vision-input path follows in the fast-follow. This is the full macOS v1 plus the cross-platform core.

## Windows heavy-rail parity (fast-follow)

Two adapters trail the 12-day line, both behind the same `DeviceController` port so no caller changes:

- **Routines on Windows:** UI Automation for the accessibility tree + SendInput for synthetic input, mirroring the macOS AX + CGEvent recorder/replay.
...
Estimated a couple of days once R3 and R4 have set the shape on macOS. Tracked as the immediate next work after day 12, not a someday item.

## Windows parity note (rail by rail)

- **Semantic rail** differs by OS: Graph / Outlook / Windows shell / To Do on Windows vs EventKit / AppleScript on macOS. iMessage is macOS-only. Cross-platform in R1.
- **Browser rail** is shared (Electron), the cheapest parity win. Cross-platform in R4.
- **Accessibility rail** needs a Windows implementation: UI Automation + SendInput vs AX + CGEvent. Fast-follow.
- **Vision rail** is largely shared except input injection (SendInput vs CGEvent). Fast-follow.
...
| Windows UI Automation act-primitives | fast-follow | the Windows analogue of the AX act-primitives |

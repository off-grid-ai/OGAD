...
7. **A cross-rail escalation is a re-fire, under the same policy.** Falling back from one rail to another (semantic timed out -> try the browser) is another execution attempt on the same Action, so it is governed by the same retry rules: only a retryable action (reversible, reliably verifiable, retry budget left) may escalate, and only after verification confirms the effect did NOT happen. A timed-out irreversible action goes to `needs_help`, never to another rail - that is how a double-send is made impossible even across rails. The durable Action record is the effect journal: every attempt records the rail it ran on.

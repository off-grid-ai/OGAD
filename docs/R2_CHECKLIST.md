...
- [x] **A1. Per-platform tool specs**: `specsForPlatform(platform)` in the logic file -
      ...
      _Done when:_ filtering + hints tested per platform; the extension's schemas/canHandle/
...
- [x] **A2. The win32 inline runner**: open/navigate on Windows goes through the shell
      ...
      _Done when:_ runner tests through the injected opener; unknown verbs refuse.
- [x] **A3. Outlook read-back verifiers**: list scripts for tasks (olFolderTasks 13)
      ...
      _Done when:_ script content + reader mapping tested; the read-back verifiers pass over

...
- Before declaring a change locally ready, commit it and push normally. The pre-push hook is the required local quality gate and owns the node, web, and applicable Pro typechecks, tests, and coverage ratchet. Never bypass it with `--no-verify`. Run the full commands separately only when needed to diagnose a failed hook.
